...
async def response_model(user: UserIn):
    ...
    print('04-用户密码',user.password)  # password不会被返回
    # return user
    return user
...
async def status_attribute():
    print('04-状态码',type(status.HTTP_200_OK))
...
@app04.post(
    ...
    summary="This is summary",               #文档标签,为NONE是函数名
    description="This is description",       #文档摘要
    response_description="This is response description",   #文档返回数据摘要
    deprecated=True,                                       #是否遗弃(任然可用)
...
"""FastAPI 应用的常见配置项【见run.py】"""




"""Handling Errors 错误处理 【见run.py】"""
@app04.get("/http_exception")
async def http_exception(city: str):
    if city != "Beijing":
        raise HTTPException(status_code=404, detail="City not found!", headers={"X-Error": "Error"}) #detail:错误信息
    return {"city": city}
