...
def path_params_validate(
    num: int = Path(..., title="Your Number", description="不可描述", ge=1, le=10)):#校验路径参数用Path()
    return num
...
def query_params_validate(
    value: str = Query(..., min_length=8, max_length=16, regex="^a"),  #校验查询参数用Query(), ...换成None就变成选填的参数
...
class CityInfo(BaseModel):
    name: str = Field(..., example="Beijing")  # #校验字段参数用Field()       example是注解的作用，值不会被验证
    country: str
    country_code: str = None  # 给一个默认值
    country_population: int = Field(default=800, title="人口数量", description="国家的人口数量", ge=800)

    class Config:
        schema_extra = {           #schema_extra 是在交互文档显示的示例:{"example":{name:xxx,}}
            "example": {
                "name": "Shanghai",
                "country": "China",
                "country_code": "CN",
