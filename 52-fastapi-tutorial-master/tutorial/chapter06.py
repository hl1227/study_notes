...
#在页面添加登录认证,认证在tokenUrl接口执行 请求Token的URL地址 http://127.0.0.1:8000/chapter06/token
oauth2_schema = OAuth2PasswordBearer(tokenUrl="/chapter06/token")  #


@app06.get("/oauth2_password_bearer")
async def oauth2_password_bearer(token: str = Depends(oauth2_schema)):
    print('06 token:',token) #没有加密返回的是用户名
    return {"token": token}
...
async def get_current_user(token: str = Depends(oauth2_schema)):
    ...
    if token in fake_users_db:
        user=UserInDB(**fake_users_db[token])
    if not user:
        raise HTTPException(
...
fake_users_db.update({
    "john snow": {
        "username": "john snow",
        "full_name": "John Snow",
        "email": "johnsnow@example.com",#secret
...
def verity_password(plain_password: str, hashed_password: str):
    ...
    # 加密演示:pwd_context.hash('password')
    """#验证接收用户密码与数据库加密用户密码"""
...
def jwt_authenticate_user(db, username: str, password: str):#验证用户
    user = jwt_get_user(db=db, username=username)
    print('用户存在:',user)
    if not user:                                            #验证用户是否存在
        return False
    verity_password_res=verity_password(plain_password=password, hashed_password=user.hashed_password)#验证接收用户密码与数据库加密用户密码
    print('验证密码:', verity_password_res)
    if not verity_password_res:
        return False
    return user

#如果用户和密码都验证成功,则创建用户的JWT-token
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    to_encode = data.copy()
    print('copy后的:',to_encode)
    if expires_delta:
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    print('接收到的密码:',form_data.password)
...
async def jwt_get_current_user(token: str = Depends(oauth2_schema)):
    ...
    try:#解密
        print('依赖oauth2_schema返回值:',token)
        payload = jwt.decode(token=token, key=SECRET_KEY, algorithms=[ALGORITHM])
        username = payload.get("sub")#获取用户名(加密用的用户名)
        if username is None:
            raise credentials_exception
    except JWTError:
        raise credentials_exception
    user = jwt_get_user(db=fake_users_db, username=username)#获取用户数据
    if user is None:
        raise credentials_exception
    return user
...
async def jwt_get_current_active_user(current_user: User = Depends(jwt_get_current_user)):
    if current_user.disabled:#判断是否是活跃用户
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Inactive user")
    return current_user
