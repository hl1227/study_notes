...
Base.metadata.create_all(bind=engine)#生成数据库表
...
def create_city(city: schemas.CreateCity, db: Session = Depends(get_db)):
    db_city = crud.get_city_by_name(db, name=city.province)#去重
    if db_city:
...
def bg_task(url, db: Session):
    """这里注意一个坑，不要在后台任务的参数中db: Session = Depends(get_db)这样导入依赖"""

    city_data = requests.get(url=str(url)+"?source=jhu&country_code=CN&timelines=false")
    if 200 == city_data.status_code:
        ...
        for location in city_data.json()["locations"]:
            ...
            print('城市创建成功:',crud.create_city(db=db, city=schemas.CreateCity(**city)))

    coronavirus_data = requests.get(url=str(url)+"?source=jhu&country_code=CN&timelines=true")
    if 200 == coronavirus_data.status_code:
        ...
        for city in coronavirus_data.json()["locations"]:
            ...
            for date, confirmed in city["timelines"]["confirmed"]["timeline"].items():
                ...
                print('数据创建成功:',crud.create_city_data(db=db, data=schemas.CreateData(**data), city_id=db_city.id))
