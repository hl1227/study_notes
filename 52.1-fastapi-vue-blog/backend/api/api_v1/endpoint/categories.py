...
@router.post('/',dependencies=[Depends(deps.get_current_active_admin)],response_model=schemas.CategoryOut, status_code=status.HTTP_201_CREATED)
def create_category(db: Session = Depends(deps.get_db), img: UploadFile = File(...), name: str = Form(...)):# *, category_in: schemas.CategoryCreate
